...
def _read_(dat, target=sys.stdout):
    ...
    for ver in meta['ver']:
        for dev in meta['dev']:
            if all((tag in dat for tag in (dev, meta['mss'], ver))):
                return dev  # reset detected
    if meta['cli'] in dat:
        return (None,)  # cli ready
    return ()  # unknown state


def _init_(dat):
    aux = serial.Serial(dat, meta['aux'], timeout=0.01)
        ...
        if 'compRangeBiasAndRxChanPhase' in c:
            ...
            if c['compRangeBiasAndRxChanPhase']['phaseBias'] is None or \
                type(c['compRangeBiasAndRxChanPhase']['phaseBias']) == list and \
                 ...
                 c['compRangeBiasAndRxChanPhase']['phaseBias'] = [1, 0] * meta['ant'][0] * meta['ant'][1]
...
def _proc_(cfg, par, err={1: 'miss', 2: 'exec', 3: 'plot'}):
    ...
    apps.clear()
    for cmd, app in meta['app'].items():
        if type(app) not in (list, tuple):
            app = (app,)
        for item in app:
...
def _data_(prt):  # observe auxiliary port and process incoming data
    ...
    input, output, sync, size = {'buffer': b''}, {}, False, meta['blk']
    ...
    while True:
        try:
            ...
            if data[:len(meta['seq'])] == meta['seq']:  # check for magic sequence
